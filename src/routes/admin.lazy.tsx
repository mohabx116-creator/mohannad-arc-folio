...
export default function Admin() {
  ...
  if (auth.status === "loading") {
    return (
      <Shell>
        <p className="px-6 py-24 text-center text-sm text-ivory/60">Verifying admin access...</p>
      </Shell>
    );
  }
      ...
      <Shell>
        <section className="mx-auto max-w-xl px-6 py-24 text-center">
          ...
          <p className="mt-4 text-sm leading-relaxed text-ivory/60">
            The CMS could not finish verifying admin access. This usually means Supabase environment
            variables, network access, or auth/RLS setup needs attention.
          </p>
          {auth.setupError && (
            <p className="mt-5 break-words text-xs leading-relaxed text-gold/80">
              {auth.setupError}
            </p>
          )}
          <div className="mt-8 border border-ivory/10 bg-ivory/[0.03] p-5 text-left text-xs leading-relaxed text-ivory/65">
            <p>
              <span className="text-gold">Failed step:</span> {auth.failedStep ?? "Unknown"}
            </p>
            <p className="mt-2 break-all">
              <span className="text-gold">Email:</span> {auth.email ?? "Not available"}
            </p>
            <p className="mt-2 break-all">
              <span className="text-gold">User ID:</span> {auth.userId ?? "Not available"}
            </p>
            <p className="mt-2">
              <span className="text-gold">Suggested fix:</span>{" "}
              {auth.suggestedFix ?? "Check Supabase Auth, user_roles, and RLS policies."}
            </p>
          </div>
          <div className="mt-8 flex justify-center gap-3">
            <button
              onClick={() => recheckAccess()}
              className="bg-ivory px-6 py-3 text-[10px] uppercase tracking-[0.3em] text-onyx hover:bg-gold"
            >
              Retry
            </button>
            <button
              onClick={signOut}
              className="border border-ivory/25 px-6 py-3 text-[10px] uppercase tracking-[0.3em] hover:border-gold hover:text-gold"
            >
              Logout
            </button>
          </div>
    ...
    return (
      <Shell>
        <section className="mx-auto flex min-h-[calc(100vh-6rem)] max-w-md flex-col items-center justify-center px-6 text-center">
          ...
          <p className="mt-4 text-sm leading-relaxed text-ivory/60">
            Manage Mohannad's architectural portfolio, project sections, media, and presentation
            content.
          </p>
          <button
            onClick={signInWithGoogle}
            disabled={loading}
            className="mt-10 w-full bg-ivory px-8 py-4 text-[11px] uppercase tracking-[0.3em] text-onyx hover:bg-gold disabled:opacity-50"
          >
          ...
          <p className="mt-5 text-[10px] uppercase tracking-[0.25em] text-ivory/35">
      ...
      <Shell>
        <section className="mx-auto max-w-xl px-6 py-24 text-center">
          ...
          <p className="mt-4 text-sm text-ivory/60">
            This account is authenticated but not authorized to access the portfolio CMS.
          </p>
      ...
      <Shell>
        <section className="mx-auto max-w-2xl px-6 py-24">
          ...
          <p className="mt-6 text-sm leading-relaxed text-ivory/60">
            Open Supabase Dashboard &gt; Table Editor &gt; user_roles and insert a row with these
            values. Automatic role creation is only attempted for the two allowed Gmail accounts and
            may be blocked by RLS until the migration is applied.
          </p>
          {auth.setupError && (
            <p className="mt-3 text-xs text-gold/70">Setup detail: {auth.setupError}</p>
          )}
  ...
  return (
    <main className="min-h-screen bg-onyx text-ivory">
      <header className="flex items-center justify-between border-b border-ivory/10 px-6 py-5 md:px-12">
        <Link
          to="/"
          className="flex items-center gap-3 text-[10px] uppercase tracking-[0.35em] text-ivory/60 hover:text-gold"
        >
...
function CmsApp({ userEmail, onSignOut }: { userEmail?: string; onSignOut: () => void }) {
  const [mode, setMode] = useState<"preview" | "projects" | "sections" | "assets" | "site">(
    "preview",
  );
  ...
  const sections = useQuery({
    queryKey: ["cms-sections", projectId],
    queryFn: () => fetchSections(projectId),
    enabled: !!projectId,
  });
  const assets = useQuery({
    queryKey: ["cms-assets", projectId],
    ...
    <Shell>
      <div className="mx-auto max-w-7xl px-6 py-8 md:px-12">
        <div className="flex flex-col justify-between gap-5 border-b border-ivory/10 pb-8 md:flex-row md:items-end">
          ...
          <button
            onClick={onSignOut}
            className="self-start border border-ivory/20 px-5 py-3 text-[10px] uppercase tracking-[0.25em] hover:border-gold hover:text-gold"
          >
            Sign out
          </button>
        </div>
        ...
        <div className="mt-8 flex flex-wrap gap-2">
          {[
            ...
            <button
              ...
              className={`inline-flex items-center gap-2 border px-4 py-2 text-[10px] uppercase tracking-[0.25em] ${mode === key ? "border-gold bg-gold text-onyx" : "border-ivory/15 hover:border-gold hover:text-gold"}`}
            >
        ...
        {projects.isLoading ? (
          ...
          <div className="mt-10">
            ...
            {mode === "site" && (
              <SitePanel rows={site.data ?? []} contact={contact.data} invalidate={invalidate} />
            )}
          </div>
        )}
      </div>
      ...
      <div className="relative min-h-[520px]">
        ...
        <div className="relative p-8 md:p-12">
          ...
          <p className="mt-5 max-w-2xl text-lg text-ivory/75">
            {project?.long_description ?? alSe7r.longDescription}
          </p>
        </div>
      </div>
    ...
    <CrudPanel
      ...
      save={async (row) => {
        ...
        const { error } = payload.id
          ? await db.from("projects").update(payload).eq("id", payload.id)
          : await db.from("projects").insert(payload);
        if (error) throw error;
        if (payload.id)
          await syncProjectHeroCover(
            payload.id,
            payload.hero_asset_id ?? null,
      ...
      fieldOptions={(editing) => ({
        hero_asset_id: buildHeroCoverOptions(
          assets.filter((asset) => asset.project_id === editing.id),
        ),
        cover_asset_id: buildHeroCoverOptions(
          assets.filter((asset) => asset.project_id === editing.id),
        ),
      })}
  ...
  return (
    <CrudPanel
      ...
      save={async (row) => {
        ...
        const { error } = payload.id
          ? await db.from("portfolio_sections").update(payload).eq("id", payload.id)
          : await db.from("portfolio_sections").insert(payload);
        if (error) throw error;
      }}
      remove={async (row) => {
...
function AssetsPanel({
  ...
  const filtered =
    typeFilter === "all"
      ? assets
      : assets.filter((asset) => asset.asset_type === typeFilter || asset.category === typeFilter);
  const groups = [
    ...sections.map((section) => ({
      id: section.id ?? "",
      title: section.title,
  ...
  const removeAsset = async (row: AssetRow) => {
    if (row.is_hero)
      await db.from("projects").update({ hero_asset_id: null }).eq("id", row.project_id);
    if (row.is_cover)
      await db.from("projects").update({ cover_asset_id: null }).eq("id", row.project_id);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap gap-2">
        {["all", "image", "pdf", "render", "plan", "elevation", "section", "poster"].map(
          (filter) => (
            <button
              key={filter}
              onClick={() => setTypeFilter(filter)}
              className={`border px-3 py-2 text-[10px] uppercase tracking-[0.22em] ${typeFilter === filter ? "border-gold bg-gold text-onyx" : "border-ivory/15 hover:border-gold"}`}
            >
              {filter}
            </button>
          ),
      ...
      <div className="space-y-8">
        {groups.map((group) => (
          <CrudPanel
            ...
            createDefaults={() => ({
              project_id: projectId,
              section_id: group.id === "unassigned" ? null : group.id,
              file_url: "",
              storage_path: "",
              ...
              category: "other",
              is_published: true,
              display_order: group.rows.length + 1,
            })}
            save={saveAsset}
            remove={removeAsset}
            fields={[
            ...
            renderActions={(row) => (
              <div className="flex flex-col gap-2">
                ...
                {(row as AssetRow).asset_type === "pdf" && (
                  <ReplacePdfThumbnail asset={row as AssetRow} invalidate={invalidate} />
                )}
              </div>
            )}
            onReorder={
              typeFilter === "all"
                ? (ordered) => updateDisplayOrder("portfolio_assets", ordered)
                : undefined
            }
          />
        ))}
...
function UploadCard({
  ...
  async function onFiles(files: FileList | null) {
    ...
    for (const [index, file] of selected.entries()) {
      ...
      const assetType = file.type.includes("pdf")
        ? "pdf"
    ...
    <div className="border border-ivory/10 p-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <select
          ...
          className="border border-ivory/20 bg-onyx px-4 py-3 text-sm"
        >
          {sections.map((section) => (
      ...
      <label className="mt-6 flex cursor-pointer flex-col items-center justify-center border border-dashed border-ivory/20 p-10 text-center hover:border-gold">
        ...
        <span className="mt-3 text-sm text-ivory/70">
          {uploading ? "Uploading..." : "Select images, PDFs, boards, drawings, or files"}
        </span>
        {uploadStatus && (
          <span className="mt-2 max-w-full truncate text-xs text-gold/80">{uploadStatus}</span>
        )}
        <input
          type="file"
          multiple
    ...
    <div className="space-y-10">
      <CrudPanel
        ...
        save={async (row) => {
          ...
          const { error } = payload.id
            ? await db.from("site_content").update(payload).eq("id", payload.id)
      ...
      <CrudPanel
        ...
        remove={async () => {
          const { error } = await db
            .from("contact_info")
            .update({ email: null, phone: null, whatsapp: null, linkedin: null })
            .eq("id", 1);
          if (error) throw error;
        }}
...
function visualRank(asset: AssetRow) {
  ...
  if (asset.category === "poster" || asset.asset_type === "poster" || asset.asset_type === "board")
    return 2;
  if (asset.asset_type === "image" || asset.mime_type?.startsWith("image/")) return 3;
  return 9;
}

async function syncProjectHeroCover(
  projectId: string,
...
async function syncAssetHeroCover(
  projectId: string,
  assetId: string,
  makeHero: boolean,
  makeCover: boolean,
...
function CrudPanel<T extends Record<string, unknown>>({
  ...
  fieldOptions?:
    ...
    | ((editing: Partial<T>) => Record<string, FieldOption[]>);
  ...
  const activeFieldOptions =
    typeof fieldOptions === "function" && editing ? fieldOptions(editing) : fieldOptions;
  ...
  return (
    <section>
        ...
        <div>
          ...
          {onReorder && (
            <p className="mt-2 text-xs text-ivory/45">
              {reordering
                ? "Saving order..."
                : "Drag the handle to reorder. Numeric order remains available as fallback."}
            </p>
          )}
        </div>
        <button
          onClick={() => setEditing(createDefaults())}
          className="inline-flex items-center gap-2 bg-ivory px-5 py-3 text-[10px] uppercase tracking-[0.25em] text-onyx hover:bg-gold"
        >
        ...
        >
          <div className="grid gap-4 md:grid-cols-2">
            {fields.map((field) => (
              <EditorField
                ...
                field={field}
                value={editing[field]}
                options={activeFieldOptions?.[field]}
          ...
          <div className="mt-6 flex gap-3">
            <button
              ...
              className="inline-flex items-center gap-2 bg-gold px-5 py-3 text-[10px] uppercase tracking-[0.25em] text-onyx"
            >
            ...
            <button
              ...
              className="border border-ivory/20 px-5 py-3 text-[10px] uppercase tracking-[0.25em]"
            >
              Cancel
            </button>
          </div>
      ...
      {orderedRows.length === 0 ? (
        <p className="border border-ivory/10 bg-ivory/[0.025] p-8 text-sm text-ivory/55">{empty}</p>
      ) : (
        <div className="grid gap-4">
          {orderedRows.map((row) => (
            ...
            >
              <button
                type="button"
                className="hidden cursor-grab border border-ivory/15 p-3 text-ivory/45 hover:border-gold hover:text-gold md:block"
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <h3 className="font-display text-2xl">
                    {row.title ?? row.display_name ?? row.key}
                  </h3>
                  {row.is_hero && (
                    <span className="border border-gold px-2 py-1 text-[9px] uppercase tracking-[0.2em] text-gold">
                      Hero
                    </span>
                  )}
                  {row.is_cover && (
                    <span className="border border-ivory/30 px-2 py-1 text-[9px] uppercase tracking-[0.2em] text-ivory/70">
                      Cover
                    </span>
                  )}
                  <span
                    className={`border px-2 py-1 text-[9px] uppercase tracking-[0.2em] ${row.is_published === false ? "border-ivory/20 text-ivory/45" : "border-gold/60 text-gold"}`}
                  >
                    {row.is_published === false ? "Hidden" : "Published"}
                  </span>
                </div>
                <p className="mt-2 text-sm leading-relaxed text-ivory/55">
                  {row.description ?? row.caption ?? row.value ?? row.short_description}
                </p>
                <p className="mt-3 text-[10px] uppercase tracking-[0.25em] text-gold">
              ...
              <div className="flex items-start gap-2">
                <button
                  ...
                  className="border border-ivory/20 p-3 hover:border-gold hover:text-gold"
                  aria-label="Edit"
                >
      ...
      {isAssetSelector && options ? (
        <div className="mt-2 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          ...
          {options.map((option) => (
            ...
            >
              {option.preview && (
                <img src={option.preview} alt="" className="h-28 w-full object-cover" />
              )}
              <span className="block px-3 pt-3 font-display text-lg">{option.label}</span>
              <span className="block px-3 pb-3 pt-1 text-[10px] uppercase tracking-[0.2em] text-ivory/40">
                {option.meta}
              </span>
            </button>
          ))}
        ...
        <select
          ...
          className="mt-2 w-full border border-ivory/20 bg-onyx px-3 py-3"
        >
        ...
        <textarea
          ...
          rows={4}
          className="mt-2 w-full border border-ivory/20 bg-onyx px-3 py-3 outline-none focus:border-gold"
        />
      ) : (
        <input
          value={value ?? ""}
          onChange={(event) =>
            onChange(field.includes("order") ? Number(event.target.value) : event.target.value)
          }
          className="mt-2 w-full border border-ivory/20 bg-onyx px-3 py-3 outline-none focus:border-gold"
        />
      )}
...
function AssetPreview({ asset }: { asset: AssetRow }) {
  ...
  if (asset.asset_type === "pdf") {
    const thumbnailUrl =
      typeof asset.metadata?.thumbnail_url === "string" ? asset.metadata.thumbnail_url : "";
    return (
      <div className="mb-4 overflow-hidden border border-ivory/10 bg-ivory">
        {thumbnailUrl ? (
          ...
          <iframe
            ...
            src={`${asset.file_url}#page=1&toolbar=0&navpanes=0&scrollbar=0`}
            className="h-40 w-full"
          />
        )}
        <div className="bg-onyx px-3 py-2 text-[10px] uppercase tracking-[0.25em] text-gold">
          PDF Preview
        </div>
      </div>
    );
